...
contract('IICO', function (accounts) {
  ...
  it('Should create the contract with the initial values', async () => {
    let startTestTime = Math.floor(Date.now() / 1000)
  ...
  it('Should withdraw the proper amount', async () => {
    ...
    expectThrow(iico.withdraw(1,{from: buyerA, gasPrice: gasPrice}))
    ...
    expectThrow(iico.withdraw(2,{from: buyerB, gasPrice: gasPrice})) // You should not be able to withdraw twice.
